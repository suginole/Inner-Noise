...
class Field:
    ...
    def get_surface(self) -> pygame.Surface:
        ...
        # バイオームに応じた色を塗る（グラデーションなし・完全割り当て）
        w_mask = h < lo
        m_mask = h >= hi
        g_mask = ~w_mask & ~m_mask

        cW = np.array(BIOME_COLORS['W'], dtype=np.uint8)
        cG = np.array(BIOME_COLORS['G'], dtype=np.uint8)
        cM = np.array(BIOME_COLORS['M'], dtype=np.uint8)

        for c in range(3):
            rgb[w_mask, c] = cW[c]   # 沼: 水色
            rgb[g_mask, c] = cG[c]   # 平地: ミルク色
            rgb[m_mask, c] = cM[c]   # 山: 深緑
